...
class XBertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.adapter_vision_1 = MM_Adapter_Layer(bottleneck=self.rank, feature_dim=self.vision_dim)
        self.adapter_vision_2 = MM_Adapter_Layer(bottleneck=self.rank, feature_dim=self.vision_dim)

        self.adapter_audio_1 = MM_Adapter_Layer(bottleneck=self.rank, feature_dim=self.audio_dim)
        self.adapter_audio_2 = MM_Adapter_Layer(bottleneck=self.rank, feature_dim=self.audio_dim)
        
        self.adapter_1 = Adapter_Layer(bottleneck=self.rank)
        self.adapter_2 = Adapter_Layer(bottleneck=self.rank)
        
        self.adapter_gate = nn.Linear(config.hidden_size+self.audio_dim+self.vision_dim, self.num_experts)
        # self.adapter = Mixture_Adapters()
    ...
    def forward(
        ...
        audio = audio.contiguous().view(-1,self.audio_dim)
        vision = vision.contiguous().view(-1,self.vision_dim)
        gate_logits = self.adapter_gate(torch.concatenate([attention_output, audio, vision], dim=-1))  # T, N
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([XBertLayer(config) for _ in range(config.num_hidden_layers)])
        # self.last_layer = BertLayer(config)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs, load_layer = torch.utils.checkpoint.checkpoint(
                ...
                layer_outputs, load_layer = layer_module(
                    hidden_states,
                    attention_mask,
                    layer_head_mask,
                    encoder_hidden_states,
